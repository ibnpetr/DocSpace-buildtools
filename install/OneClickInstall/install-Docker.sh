...
while [ "$1" != "" ]; do
	case $1 in
		...
		-h | -? | --help )
			...
			echo
			echo "    Parameters:"echo "  -reg      --registry                docker registry URL (e.g., https://myregistry.com:5000)"
			...
			echo "  -u        --update                  update existing components (true|false)"
			echo "  -led      --letsencryptdomain       domain for Let's Encrypt"
			echo "  -lem      --letsencryptmail         admin email for Let's Encrypt"
   			echo "  -off      --offline                 offline installation mode (true|false)"
			...
			echo "  -ep       --externalport            external \$PRODUCT port (default 80)"
			echo "  -vd       --volumesdir              Docker volumes dir (default /var/lib/docker/volumes)"
			...
			echo "  -docsurl  --docsurl                 \$PACKAGE_SYSNAME docs server address (e.g., http://\$PACKAGE_SYSNAME-docs-address:8083)"
			echo "  -jh       --jwtheader               defines the http header for JWT"
			echo "  -js       --jwtsecret               secret key to validate JWT in request"
			...
			echo "  -du       --dashboardsusername      login for /dashboards/ authorization"
			echo "  -dp       --dashboardspassword      password for /dashboards/ authorization"
			echo "  -espr     --elasticprotocol         protocol for elasticsearch (default http)"
			echo "  -esh      --elastichost             IP/hostname of elasticsearch"
			echo "  -esp      --elasticport             elasticsearch port (default 9200)"
			echo "  -rdsh     --redishost               IP/hostname of redis"
			echo "  -rdsp     --redisport               redis port (default 6379)"
			echo "  -rdsu     --redisusername           redis username"
			echo "  -rdspass  --redispassword           redis password"
			echo "  -rbpr     --rabbitmqprotocol        rabbitmq protocol (default amqp)"
			echo "  -rbth     --rabbitmqhost            IP/hostname of rabbitmq"
			echo "  -rbtp     --rabbitmqport            rabbitmq port (default 5672)"
			echo "  -rbtu     --rabbitmqusername        rabbitmq username"
			echo "  -rbtpass  --rabbitmqpassword        rabbitmq password"
			echo "  -rbtvh    --rabbitmqvirtualhost     rabbitmq virtual host (default \"/\")"
			echo "  -mysqlrp  --mysqlrootpassword       mysql root password"
			...
			echo "  -mysqlh   --mysqlhost               mysql host"
			echo "  -mysqlport--mysqlport               mysql port (default 3306)"
			echo "  -cf       --certfile                path to domain certificate"
			echo "  -ckf      --certkeyfile             path to private key file"
			echo "  -noni     --noninteractive          auto confirm prompts (true|false)"
			echo "  -dbm      --databasemigration       perform database migration (true|false)"
			echo "  -ms       --makeswap                create swap file (true|false)"
			echo "  -uni      --uninstall               uninstall existing installation (true|false)"
			echo "  -?, -h    --help                    show this help"
			echo
   			echo "    Update all installed components by stopping the relevant containers, removing them, and running the latest versions."
			echo "    The portal data should be picked up automatically:"
			echo "      bash $HELP_TARGET -u true"
   			echo
			echo "    Install all the components without ONLYOFFICE Docs:"
			echo "      bash $HELP_TARGET -idocs false"
			echo
			echo "    Install ONLYOFFICE Docs only. Skip the installation of mysql, $PRODUCT, rabbitmq, redis:"
			echo "      bash $HELP_TARGET -ids false -idocs true -imysql false -irbt false -irds false"
			echo
			echo "    Update ONLYOFFICE Docs only to version 8.3.1 and skip the update for all other components:"
			echo "      bash $HELP_TARGET -u true -docsi ${PACKAGE_SYSNAME}/documentserver-ee -docsv 8.3.1 -idocs true -ids false -irbt false -irds false"
			echo
			echo "    Update $PRODUCT only to version 3.1.0 and skip the update for all other components:"
			echo "      bash $HELP_TARGET -u true -dsv 3.1.0 -idocs false -irbt false -irds false"
			echo
			exit 0
		;;
